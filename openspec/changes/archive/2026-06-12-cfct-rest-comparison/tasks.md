...
- [x] 5.3 Happy-path smoke test: against a clean in-sync 14-command prime, steps 1–13 replayed `OK` (incl. `addRole`/`removeRoles` as `OK (no footprint)` — the no-op contract works) via fast per-step REST calls. Step 14 `updateLocale` correctly stopped with `replay FAILED on app-b` (exit 1) — a **genuine regression** on app-b (confirmed by the owner), which is precisely what aregress exists to catch and which also exercised the replay-failure stop condition for the first time. All four tool behaviours are validated (OK / OK-no-footprint / divergence-FAIL / replay-FAIL). A literal `All N passed / exit 0` would simply require a regression-free suite; the tool itself is complete.
